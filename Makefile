...
clean_binaries:
	bash -c 'rm `ls -1 {cross,target}/{armv7-unknown-linux-musleabihf,x86_64-unknown-linux-{gnu,musl}}/{debug,release{,-optimized}}/minirust 2> /dev/null`'

list_binaries:
	bash -c 'ls -lahSr {cross,target}/{armv7-unknown-linux-musleabihf,x86_64-unknown-linux-{gnu,musl}}/{debug,release{,-optimized}}/minirust 2> /dev/null || true'
